...
struct time_cmp{ //重载优先队列为按照访问时间升序
...
bool block_find(int t,int x){ //检查物理块中有没有当前页面
    if(t==1)return false;
    for(int i=1;i<=phy;i++)
        if(sim[i][t-1]==x)return true;
    return false;
}
...
void fifo(){
    ...
    int miss=0;
    for(int i=1;i<=20;i++){
            if(i!=1){//先将物理块中的序列复制一下
                for(int l=1;l<=phy;l++)
                    sim[l][i]=sim[l][i-1];
            }
        if(block_find(i,Block[i])){ //如果物理块中已经有了本页面 无需置换
            continue;
        }
...
//LRU
void lru(){//使用优先队列模拟大根堆，采用最近访问时间排序
    printf("\n采用LRU页面置换算法\n");
    memset(sim,-1,sizeof(sim));
    ...
    for(int i=1;i<=20;i++){
        ...
        else if(book<phy)//如果当前物理块中不满 也无需置换
        {
            ...
            ans=lru_p.top();
            lru_p.pop();
            while(del[ans.no]!=ans.time){ //检查是否被懒惰删除
                ans=lru_p.top();
                lru_p.pop();
            }
            for(int l=1;l<=phy;l++){
...
void opt(){
    ...
    for(int i=1;i<=20;i++){
        ...
        for(int l=1;l<=phy;l++){//寻找未来最久没被访问且在当前物理块的页面号
